...
export const AgentRosterCard: React.FC<Props> = ({ agent, selected, onClick }) => {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`agent-roster-card relative w-full overflow-hidden rounded-xl p-4 text-left ${selected ? 'selected' : ''}`}
    >
      {selected && (
        <div className="pointer-events-none absolute -right-8 -top-8 h-24 w-24 rounded-full bg-cyan-500/15 blur-xl" />
      )}
      <div className="relative z-10 flex items-start justify-between gap-2">
        <div className="flex min-w-0 items-center gap-3">
          <div className="relative flex h-12 w-12 shrink-0 items-center justify-center rounded-full border border-cyan-500/30 bg-gray-100 dark:bg-gray-900">
            {selected && isActive && (
              <div className="absolute inset-0 rounded-full border border-cyan-400 agent-pulse-ring" />
            )}
            <MaterialIcon name="face_4" className="text-cyan-500 dark:text-cyan-400" />
          </div>
