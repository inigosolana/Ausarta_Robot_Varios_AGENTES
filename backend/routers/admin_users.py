"""Gestión de usuarios e impersonación (admin)."""
...
from services.admin_helpers import (
    canonical_impersonation_role,
    canonical_role,
    resolve_master_empresa_id,
    sign_impersonation_payload,
)
from services.audit import log_audit_event
from services.auth import CurrentUser, require_admin, require_superadmin
from services.platform_access import has_global_access, is_ausarta_platform_admin
from services.profile_cache import invalidate_user_profile_cache
from services.rate_limiter import limiter
from services.supabase_service import clear_ui_cache, sb_query, supabase
...
async def impersonate_tenant(request: Request, payload: dict, current_user: CurrentUser = Depends(require_superadmin)):
    ...
    desired_role = canonical_impersonation_role(payload.get("role"))
    ...
    token = sign_impersonation_payload(token_payload)
...
async def create_auth_user(request: Request, payload: dict, current_user: CurrentUser = Depends(require_admin)):
    ...
    master_empresa_id = resolve_master_empresa_id()
...
async def delete_auth_user(request: Request, user_id: str, current_user: CurrentUser = Depends(require_admin)):
    ...
    try:
        ...
        target = prof.data[0]
        target_role = canonical_role(target.get("role"))
...
async def update_user(
    ...
    if "role" in payload:
        new_role = canonical_role(payload["role"])
